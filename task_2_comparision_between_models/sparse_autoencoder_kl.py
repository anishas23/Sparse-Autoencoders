...
# KL Divergence-based Sparse Autoencoder
...
def kl_divergence(p, q):
    return torch.sum(p * torch.log(p / q) + (1-p) * torch.log((1-p) / (1-q)))

def train_sparse_autoencoder_kl():
    ...
    model = SparseAutoencoderKL().to(device)
    ...
    for epoch in range(epochs):
        ...
        for imgs, _ in train_loader:
            ...
            kl_loss = kl_divergence(torch.full_like(rho_hat, rho), rho_hat)
